...
{
	double pi = 4.0*atan(1.0);
	return  exp(-d_plus(S0, sigma, r)*d_plus(S0, sigma, r) / 2) / (S0 * sigma *sqrt(T) * sqrt(2.0*pi));
}
double EurCall::DeltaByBSFormula(double S0, double sigma, double r,double S0_)
{
	return S0 * N(d_plus(S0, sigma, r)) / S0_;
}